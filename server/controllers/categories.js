...
exports.getArticleListByTagName = function(req, res, next) {
	...
	query_str = 'select sql_calc_found_rows a.id,a.title,a.userid,a.content,a.desc,UNIX_TIMESTAMP(a.create_time) as create_time,a.tags,b.name from article a left join user b on a.userid=b.id where tags like concat("%",'+tagName+',"%") order by a.create_time limit ' + start + ',' + pageSize;
	dao.query(query_str).done(function(data) {
		if (data.res == 0) {
			dao.query('select found_rows() as total').done(function(result) {
				if (result.res == 0) {
