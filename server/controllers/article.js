...
exports.getArticleList = function(req, res, next) {
	var pageIndex = req.params.pageIndex || 1,
		...
		query_str = 'select sql_calc_found_rows md5(a.id) as id,a.title,a.userid,a.content,a.desc,UNIX_TIMESTAMP(a.create_time) as create_time,a.tags,b.name from article a left join user b on a.userid=b.id order by a.create_time desc limit ' + start + ',' + pageSize,
		result = {
			res: -1,
			data: null,
			msg: ''
...
exports.getArticle = function(req, res, next) {
	var id = req.params.id,
		query_str = 'select sql_calc_found_rows a.id,a.title,a.userid,a.content,a.desc,UNIX_TIMESTAMP(a.create_time) as create_time,a.tags,b.name from article a left join user b on a.userid=b.id where md5(a.id)=?',
		params = [id];
...
exports.getNextAndPrev = function(req, res, next) {
	var id = req.params.id,
		query_str_pre = 'select * from article where id=(select id from article where id < ? order by create_time desc limit 1)',
		query_str_next = 'select * from article where id=(select id from article where id > ? order by create_time desc limit 1)',
		params = [id];
